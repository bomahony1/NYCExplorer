...
function HomePage() {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:8000/api/events/') // Fetch data from the backend server
      .then(response => response.json())
      .then(data => {
        if (data.length > 0) {
          const event = data[0];
          setEventTitle(event.name);
          setEventLocation(event.latitude);
          setEventURL(event.url);
          setEventStartTime(event.time);
          setEventImage(event.image);
        } else {
          setEventTitle('No event available');
          setEventLocation('Location not available');
          setEventURL('');
          setEventStartTime('');
          setEventImage('');
        }
